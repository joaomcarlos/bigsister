...
namespace Magic
{
	...
	public static class SMemory
	{
		...
		public static byte ReadByte(IntPtr hProcess, uint dwAddress)
		{
			...
			if (buf == null)
				throw new Exception("ReadByte at 0x" + dwAddress.ToString("X") + " failed.");
		...
		public static sbyte ReadSByte(IntPtr hProcess, uint dwAddress)
		{
			...
			if (buf == null)
				throw new Exception("ReadSByte at 0x" + dwAddress.ToString("X") + " failed.");
		...
		public static ushort ReadUShort(IntPtr hProcess, uint dwAddress, bool bReverse)
		{
			...
			if (buf == null)
				throw new Exception("ReadUShort at 0x" + dwAddress.ToString("X") + " failed.");
		...
		public static short ReadShort(IntPtr hProcess, uint dwAddress, bool bReverse)
		{
			...
			if (buf == null)
				throw new Exception("ReadShort at 0x" + dwAddress.ToString("X") + " failed.");
		...
		public static uint ReadUInt(IntPtr hProcess, uint dwAddress, bool bReverse)
		{
            ...
            if (buf == null)
                throw new Exception("ReadUInt at 0x" + dwAddress.ToString("X") + " failed.");
		...
		public static int ReadInt(IntPtr hProcess, uint dwAddress, bool bReverse)
		{
			...
			if (buf == null)
				throw new Exception("ReadInt at 0x" + dwAddress.ToString("X") + " failed.");
		...
		public static UInt64 ReadUInt64(IntPtr hProcess, uint dwAddress, bool bReverse)
		{
			...
			if (buf == null)
				throw new Exception("ReadUInt64 at 0x" + dwAddress.ToString("X") + " failed.");
		...
		public static Int64 ReadInt64(IntPtr hProcess, uint dwAddress, bool bReverse)
		{
			...
			if (buf == null)
				throw new Exception("ReadInt64 at 0x" + dwAddress.ToString("X") + " failed.");
		...
		public static float ReadFloat(IntPtr hProcess, uint dwAddress, bool bReverse)
		{
			...
			if (buf == null)
				throw new Exception("ReadFloat at 0x" + dwAddress.ToString("X") + " failed.");
		...
		public static double ReadDouble(IntPtr hProcess, uint dwAddress, bool bReverse)
		{
			...
			if (buf == null)
				throw new Exception("ReadDouble at 0x" + dwAddress.ToString("X") + " failed.");
        ...
        public static string ReadUTF8String(IntPtr hProcess, uint dwAdress, int nLength)
        {
            var buffer = new byte[nLength * 2];
            int lpNumberOfBytesRead;
            Imports.ReadProcessMemory(hProcess, dwAdress, buffer, buffer.Length, out lpNumberOfBytesRead);
        ...
        /// <summary>
        /// Writes a UTF-8 string to another process' memory.
        /// </summary>
        /// <param name="hProcess">Handle to the process to which value will be written.</param>
        /// <param name="dwAddress">Address to which value will be written.</param>
        /// <param name="Value">Value that will be written to memory.</param>
        /// <returns>Returns true on success, false on failure.</returns>
        public static bool WriteUTF8String(IntPtr hProcess, uint dwAddress, string Value)
        {
            byte[] Buffer = Encoding.UTF8.GetBytes(Value);
            return WriteBytes(hProcess, dwAddress, Buffer);
        }
		#endregion
        ...
        public static bool IsValid(IntPtr hProcess, uint dwAddress)
        {
            var mbi = new MEMORY_BASIC_INFORMATION();

            return
                Imports.VirtualQueryEx(hProcess, dwAddress, out mbi, Marshal.SizeOf(mbi)) != 0 &&
                mbi.RegionSize != 0 &&
                mbi.BaseAddress != 0 &&
                mbi.State == MemoryAllocType.MEM_COMMIT;
        }
	}
    ...
    /// Used to manipulate an external process' memory.
    /// </summary>
    public static class CopyOfSMemory
    {
        /// <summary>
        /// Number of bytes used to represent an ASCII character.
        /// </summary>
